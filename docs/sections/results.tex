...
\end{figure*}
\Cref{fig:trajectory-posenet} show a visual representation of the model performances. It is easy to notice that even if the predictions are almost in the same map zone with respect to their ground truth, they are still pretty sparse. The mean error between prediction and target is $2000 cm$.
...
Since this approach has been shown to still include some imperfections, we have also included a post-processing algorithm which tries to correct the model prediction. To be more specific, if the model prediction is not in a walkable spot in the map, then the output is going to be the nearest walkable point on the map according to the Euclidean distance criterion. The postprocessing allows to go from a mean error between prediction and target of $153 cm$ to $130 cm$. The result can be viewed in \cref{fig:trajectory-mapnet-walkable}.
